...
class Font:
    ...
    def make_font_select(self):
        ...
        self.fonts = Listbox(frame, width=35, font=('Arial', 10))
        label = Label(frame, text='Font-Family:', font=('Arial', 10))
    ...
    def make_font_style(self):
        frame = Frame(self.window)
        label = Label(frame, text='Font-Style:', font=('Arial', 10))
        label.pack(side=TOP)
        ...
        for i in font_styles:
            radio.append(Radiobutton(frame, variable=self.radio_var, text=i.title(), value=i, font=('Arial', 10)))
    ...
    def make_font_size(self):
        var = StringVar()
        frame = Frame(self.window)
        label = Label(frame, text='Font-Size:', font=('Arial', 10))
        self.font_size = Spinbox(frame, textvariable=var, from_=1, to=100, font=('Arial', 10))
    ...
    def make_buttons(self):
        frame = Frame(self.window)
        save = Button(frame, text='Save', command=self.save_font, font=('Arial', 11))
        cancel = Button(frame, text='Cancel', command=self.window.destroy, font=('Arial', 11))
